...
The optional multiline inputs `left`, `right`, `top`, and `bottom` provide independent regional ideas for `pony_v7`, `z_image`, `anima`, `wan2_2_video`, and `krea2`. They can be used without `word_salad` or `style_anchor` on these natural-prompt profiles. The positions are creative composition guidance rather than rigid geometry, so Ollama may improve transitions and overlap while preserving the broad orientation. After generation, a region counts as integrated only when one sentence contains both its direction and relevant content words. Only missing regions receive a short positional safety sentence at the beginning of `background_prompt`, in left/right/top/bottom order. `word_salad` remains global guidance that may influence every region; Pony v6 and Illustrious ignore the spatial inputs.
